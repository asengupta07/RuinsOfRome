...
const COLORS = ["blue", "green", "brown", "hazel", "gray", "amber", "purple", "pink", "red", "orange", "yellow", "white", "black"];
...
const facialAttributesGenerator = (gender: "male" | "female") => {
    ...
    const randomHairLength = HAIR_LENGTHS[Math.floor(Math.random() * HAIR_LENGTHS.length)];
    const randomHairType = HAIR_TYPES[Math.floor(Math.random() * HAIR_TYPES.length)];
    ...
    if (gender === "male") {
        const randomFacialHair = FACIAL_HAIR_TYPES[Math.floor(Math.random() * FACIAL_HAIR_TYPES.length)];
        const randomFacialHairColor = COLORS[Math.floor(Math.random() * COLORS.length)];
        return {
            ...facialFeatures,
...
const bodyAttributesGenerator = () => {
    ...
    const randomBodyHeight = BODY_HEIGHTS[Math.floor(Math.random() * BODY_HEIGHTS.length)];
    const randomBodyWeight = BODY_WEIGHTS[Math.floor(Math.random() * BODY_WEIGHTS.length)];
    return {
...
export async function POST(request: NextRequest) {
    ...
    if (!characterLore.description || !characterLore.backstory) {
        ...
        if (!characterLore.description || !characterLore.backstory) {
            return NextResponse.json({ error: "Failed to generate character after retry" }, { status: 500 });
        }
    }
    const moveset = [
