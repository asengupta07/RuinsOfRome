...
export async function GET(request: NextRequest) {
  ...
  if (roll < 2) {
    ...
    randomCelestial =
      tier1Celestials[Math.floor(Math.random() * tier1Celestials.length)];
  } else if (roll < 35) {
    // 33% chance for tier 2
    const tier2Celestials = celestials.filter((c) => c.tier === 2);
    randomCelestial =
      tier2Celestials[Math.floor(Math.random() * tier2Celestials.length)];
  } else {
    // 65% chance for tier 3
    const tier3Celestials = celestials.filter((c) => c.tier === 3);
    randomCelestial =
      tier3Celestials[Math.floor(Math.random() * tier3Celestials.length)];
  }
  ...
  return NextResponse.json({
    ...
    properties: {
      category: "celestial",
      rarity_score:
        randomCelestial.tier === 1
          ? 90 + Math.floor(Math.random() * 10)
          : randomCelestial.tier === 2
